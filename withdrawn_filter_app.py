...
if uploaded_file:
    ...
    for ref_no, group in grouped:
        ...
        # Exclusion check
        has_excluded = any(ek in status for ek in excluded_keywords for status in statuses)

        # Inclusion type checks
        has_withdrawn = any("withdrawn" in status and "client need to withdraw" not in status for status in statuses)
        has_rejected = any("rejected" in status for status in statuses)
        has_client_withdraw = any("client need to withdraw" in status for status in statuses)

        # Assign based on clean segregation
        if not has_excluded:
            if (has_withdrawn or has_rejected) and not has_client_withdraw:
                withdrawn_or_rejected_refs.append(ref_no)
            elif has_client_withdraw and not (has_withdrawn or has_rejected):
                client_withdraw_refs.append(ref_no)
